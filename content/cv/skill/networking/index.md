...
Some of it is downright boring, but I've had a ton of fun learning how the zero-trust service catalogs and meshes like Consul, reverse proxies like Caddy, Traefik, and Nginx, and service orchestrators like Nomad, ECS, and Kubernetes all work together. The ability for these tools to communicate when a new version, or additional instances of a service have joined the mesh, know how to route to them, and to create an encrypted tunnel between them is fascinating and beguilingly simple.

At my previous job, I had the opportunity to stand up an HCP Consul cluster and integrate it into our Kubernetes and ECS services. In order to study up on how Consul worked, prior to the start of the project, I set up my HashiStack homelab cluster backed by Raspberry Pi's.
